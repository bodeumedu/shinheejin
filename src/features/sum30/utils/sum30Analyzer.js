// Summary 처리 유틸리티 - OpenAI API 사용 (30개 단어)
...
export async function summarizeText(text, apiKey) {
  ...
  const prompt = `You are an English teacher creating a formal, academic summary. Create a summary sentence that:
1. Contains exactly 30 words total
2. Clearly expresses the main topic and central argument of the passage in a formal, academic tone
3. Uses formal and scholarly language appropriate for academic writing
4. Avoids conversational phrases like "The passage suggests that", "It seems that", or similar informal expressions
5. Directly states the key ideas and themes in a precise, objective manner
6. Uses sophisticated vocabulary suitable for high school to college level English

Important: 
- The sentence must be exactly 30 words total. Count carefully.
- Use formal, academic language throughout
- Focus on clearly expressing the main topic and core message
- Write in third-person, objective style

Return ONLY the complete 30-word formal summary sentence, no additional explanation or commentary.
