...
function Sum40Viewer({ data, hideAnswerPage = false, theme = 'classic', idPrefix = 'sum40' }) {
  ...
  return (
    <div className="sum40-viewer" style={getSum15ThemeCssVars(theme)}>
      ...
      {!hideAnswerPage && (
        <div id={`${idPrefix}-answer-page`} className="sum40-page sum40-answer-page">
          <div className="sum40-page-content">
            <div className="sum40-answer-title">답지</div>
            <div className="sum40-answer-content">
              {data.results
                .filter((r) => !r.error)
                .map((r, idx) => (
                  <div key={`answer-${idx}`} className="sum40-answer-item">
